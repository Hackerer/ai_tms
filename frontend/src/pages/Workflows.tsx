...
const KanbanCard = ({ request }: any) => {
    ...
    return (
        <div
            className="glass-card p-4 rounded-xl border-white/[0.05] hover:border-primary/40 transition-all hover:translate-y-[-2px] group cursor-pointer shadow-sm mb-4"
        ...
        >
            ...
            <div className="flex items-center justify-between pt-3 border-t border-white/5">
                <div className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded-full bg-gradient-to-br from-primary/20 to-purple-500/20 flex items-center justify-center border border-white/10">
                ...
                <div className="flex items-center gap-2 text-[10px] text-muted-foreground font-medium">
                    <span className="px-1.5 py-0.5 rounded bg-white/5 border border-white/10">3</span>
                </div>
            </div>
        </div>
    );
        ...
        <div className="flex flex-col w-80 shrink-0 select-none">
            <div className="flex items-center justify-between mb-4 px-2">
                <div className="flex items-center gap-2">
                    ...
                    <span className="ml-2 px-1.5 py-0.5 rounded-full bg-white/5 border border-white/10 text-[10px] font-mono text-muted-foreground">
                        {requests.length}
                    </span>
                </div>
            </div>
            <div className="flex-1 bg-white/[0.01] border border-white/5 rounded-2xl p-3 overflow-y-auto no-scrollbar min-h-[500px]">
                {requests.map((req: any) => (
                    <KanbanCard key={req.id} request={req} />
                ))}
                {status === 'Draft' && (
                    <button className="w-full py-4 border-2 border-dashed border-white/5 rounded-xl text-muted-foreground hover:border-white/10 hover:text-foreground transition-all flex flex-col items-center gap-1 group">
...
export const Workflows = () => {
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden bg-background">
            {/* Header */}
            <div className="h-16 border-b border-white/10 flex items-center justify-between px-8 bg-black/20 backdrop-blur-md z-10 shadow-sm">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex rounded-lg bg-white/5 p-1 border border-white/10">
                        <button className="px-3 py-1 rounded-md bg-white/10 text-xs font-semibold shadow-sm">看板</button>
